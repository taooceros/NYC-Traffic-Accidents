...
```{r}
# rename ave to avenue, st/sreet to street
accident_12_19 <- accidents %>% 
  ...
  mutate(`STREET NAME` = stri_replace(`STREET NAME`, regex="ave$", "avenue"),
         `STREET NAME` = stri_replace(`STREET NAME`, regex=".*st$", "street"), 
         `STREET NAME` = stri_replace(`STREET NAME`, regex=".*sreet$", "street")) 
...
# accident_12_19_cnt <- accident_12_19 %>% 
#   group_by(`STREET NAME`) %>% 
#   summarise(cnt_acc = n())

accident_w_flow <- accident_12_19 %>% 
  right_join(traffic, by=c("Date", "STREET NAME"))

df_cnt <- accident_12_19_cnt %>% 
  full_join(traffic_cnt, by="STREET NAME")

accident_w_flow <- accident_w_flow %>% 
  mutate(indicator = case_when(
    (`NUMBER OF PERSONS INJURED` == 0) & (`NUMBER OF PERSONS KILLED` == 0) ~ 0,
    TRUE ~ 1
  )) %>% 
  select(indicator, everything())

# accident_w_flow_cnt <- accident_w_flow %>% 
#   count(indicator)


accident_final<-accident_w_flow%>%
  mutate(hour = hour(`CRASH TIME`),
         flow = case_when(hour >= 0 & hour < 1 ~ `12:00-1:00 AM_1_1`,
                          hour >= 15 & hour < 16 ~ `3:00-4:00PM_1_1`))
