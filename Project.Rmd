...
test <- combined_set[[2]]
testAccuracy <- function(fit.model, train, test, var.index) {
    num <- nrow(test)
    ...
    for (i in 1:num) {
        errors[i] <- (prediction[i] - test$wt[i])^2
    }
...
(accidents_full[sample(nrow(accidents_full), 100000),]) %>%
...
accidents_full %>%
    ...
    ggplot(
        aes(x = `prcp_range`, y = injury), fill='navy'
    ) +
    geom_col()
...
(accidents_full) %>%
    ...
    summarise(injury = mean(`NUMBER OF PERSONS INJURED`, na.rm = T)) %>%
    ggplot(
        aes(x = `snow_range`, y = injury), fill='navy'
    ) +
    geom_col()
...
accidents_full %>%
    summarise(
        ...
        no_fog = sum(ifelse(Foggy+Heavy.Fog+Smoke, 0, 1), na.rm = T)/n(),
    ) %>%
    pivot_longer(
        everything(),
        names_to= "fog_type",
    ...
    ggplot() +
    geom_col(aes(x=fog_type, y=Proportion),fill = "steelblue") +
    theme_bw() +
...
accidents_full %>%
    ...
    ggplot() +
    geom_col(aes(x=fog_type, y=Proportion), fill = "steelblue", width = 0.5) + 
    theme_bw() +
...
accidents_full %>%
    summarise(
        No_Fog = mean(`NUMBER OF PERSONS INJURED` * 
            (ifelse(Foggy+Heavy.Fog+Smoke,0,1)), na.rm = T),
        Fog = mean(`NUMBER OF PERSONS INJURED` * Foggy, na.rm = T),
        Heavy.Fog = mean(`NUMBER OF PERSONS INJURED` * Heavy.Fog, na.rm = T),
        Smoke = mean(`NUMBER OF PERSONS INJURED` * Smoke, na.rm = T),
        Ice.Fog = mean(`NUMBER OF PERSONS INJURED` * Ice.Fog, na.rm = T)
    ) %>%
    pivot_longer(
        everything(),
        names_to= "fog_type",
    ...
    ggplot(aes(fog_type, mean_injury)) + 
...
accidents_full %>%
    group_by(DayLight) %>%
    summarise(n=n(), injury = mean (`NUMBER OF PERSONS INJURED`)) %>%
    drop_na() %>%
...
lm1 <- lm(`NUMBER OF PERSONS INJURED` ~ Foggy * Heavy.Fog + Smoke + `VEHICLE TYPE CODE 1` + TMAX + TMIN + `CONTRIBUTING FACTOR VEHICLE 1` + PRCP + SNOW + Ice.Fog + DayLight, data = accidents_full)

lm2 <- glm(I(`NUMBER OF PERSONS INJURED` > 0) ~ Foggy + Heavy.Fog + Smoke + `VEHICLE TYPE CODE 1` + TMAX + TMIN + `CONTRIBUTING FACTOR VEHICLE 1` + PRCP + SNOW + DayLight, data = accidents_full, family = "binomial")
...
```{r}
mean((accidents_full$`NUMBER OF PERSONS INJURED` > 0) == (predict(lm2, type="response")>0.5 ), na.rm = TRUE)
```