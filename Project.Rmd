...
lm1 <- lm(`NUMBER OF PERSONS INJURED` ~ Foggy * Heavy.Fog + Smoke + `VEHICLE TYPE CODE 1` +
    TMAX + TMIN + `CONTRIBUTING FACTOR VEHICLE 1` + PRCP + SNOW + Ice.Fog + DayLight, data = accidents_full)
...
```{r}
mean((accidents_full$`NUMBER OF PERSONS INJURED` > 0) ==
    (predict(lm2, type = "response") > 0.5), na.rm = TRUE)
```