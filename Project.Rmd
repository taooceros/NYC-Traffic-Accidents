...
af <- accidents_full %>%
    select(Foggy, Smoke, Heavy.Fog, `VEHICLE TYPE CODE 1`, TMAX, TMIN, `CONTRIBUTING FACTOR VEHICLE 1`, PRCP, SNOW, Ice.Fog, DayLight, vehicle, `BOROUGH`, `LOCATION_SPLIT`, `CROSS STREET`, `NUMBER OF PERSONS INJURED`, `LONGITUDE`, `LATITUDE`) %>%
    drop_na()
set.seed(123)
...
lm_1 <- lm(
    `NUMBER OF PERSONS INJURED` ~ Foggy + Smoke + Heavy.Fog + Foggy * Heavy.Fog + `VEHICLE TYPE CODE 1` + TMAX + TMIN + `CONTRIBUTING FACTOR VEHICLE 1` + PRCP + SNOW + Ice.Fog + DayLight + vehicle + `BOROUGH` + `LOCATION_SPLIT` + `CROSS STREET`,
    data = (train_df)
)
