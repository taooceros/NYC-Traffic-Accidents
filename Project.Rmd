...
lm2 <- glm(
    `NUMBER OF PERSONS INJURED` ~ Foggy + Heavy.Fog + Smoke + `VEHICLE TYPE CODE 1` + `CONTRIBUTING FACTOR VEHICLE 1` + PRCP + SNOW + DayLight,
    data = trainingAndTestingSet[[1]],
    family = "quasipoisson"
)
...
test.predict <- (tibble(injury = (predict(lm2,
    newdata = trainingAndTestingSet[[2]], type = "response"
) > 0.5)) %>% drop_na())$injury

test.actual <- (((trainingAndTestingSet[[2]] %>%
    drop_na(Foggy, Heavy.Fog, Smoke, `VEHICLE TYPE CODE 1`, `CONTRIBUTING FACTOR VEHICLE 1`, PRCP, SNOW, DayLight))
